...
```
2. Задание со звездочкой* Запустить реплику сразу с начальным данными (БД):  
Для запуска реплики с исходными данными на мастере потребуется:  
2.1 перевести мастер в read_only  
2.2 сделать дамп БД  
2.3 восстановить дамп на реплике  
2.4 запустить реплику, и выключить read_only на мастере.  

3. Задание со звездочкой* Настроить выборочную репликацию — исключить несколько БД или таблиц.    
3.1 Для исключения БД из репликации требуется в my.cnf установить параметр - binlog-ignore-db = dbname  
3.2 Для исключения таблицы из репликации в my.cnf установить параметр replicate-ignore-table = dbname.tablename  

Результат:    
![gtid](https://github.com/thornix/otus_dba/blob/main/hw19_mysql_replication/gtid.png)


![master-slaver-mysql](https://github.com/thornix/otus_dba/blob/main/hw19_mysql_replication/master-slaver-mysql1.png)
